...
// NarfexToken with Governance.
contract NarfexToken is BEP20 {
    // Burn tax rate in basis points. (defaut 1%, max 1%)
    ...
    event NarfexSwapRouterUpdated(
        address indexed operator,
        address indexed router,
        address indexed pair
    );
    event ExcludedFromAntiWhale(
        address indexed exludedAdresse,
        bool indexed excludedStatut
    );
    event ExcludedFromTrsfTax(
        address indexed exludedAdresse,
        bool indexed excludedStatut
    );
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "Narfex::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    /// @dev overrides transfer BEP20 function to meet tokenomics of Narfex
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
        ...
        if (
            ...
            _excludedFromTrsfTax[sender] == true ||
            _excludedFromTrsfTax[recipient] == true
        ) {
            ...
            require(
                ...
                "Narfex::transfer: Burn value invalid"
            );

            super._transfer(sender, BURN_ADDRESS, burnAmount);
            _moveDelegates(
                _delegates[sender],
                _delegates[BURN_ADDRESS],
                burnAmount
            );
     ...
     */
    function isExcludedFromTrsfTax(address _account)
        public
        view
        returns (bool)
    {
    ...
    function updateBurnRate(uint16 _burnRateTax) public onlyOperator {
        require(
            _burnRateTax <= 200,
            "Narfex::updateBurnRate: Burn rate must not exceed the maximum rate."
        );
        ...
        onlyOperator
    {
        require(
            ...
            "Narfex::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
     ...
     * Can only be called by the current operator.
     */
    function setExcludedFromTrsfTax(address _account, bool _excluded)
        public
        onlyOperator
    {
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(
            ...
            "Narfex::transferOperator: new operator is the zero address"
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "Narfex::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "Narfex::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "Narfex::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "Narfex::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Narfexs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "Narfex::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    //a way to get back other BEP20 tokens sended by error into the Narfex token contract
